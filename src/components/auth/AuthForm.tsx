"use client";

import Link from "next/link";
import { useActionState } from "react";
import type { AuthState } from "@/lib/auth/actions";
import { BRAND } from "@/lib/site";

type Action = (
  state: AuthState | undefined,
  formData: FormData,
) => Promise<AuthState>;

export default function AuthForm({
  mode,
  action,
  next,
  googleEnabled,
  authEnabled,
  initialError,
}: {
  mode: "login" | "signup";
  action: Action;
  next: string;
  googleEnabled: boolean;
  authEnabled: boolean;
  initialError?: string;
}) {
  const [state, formAction, pending] = useActionState(action, undefined);
  const isSignup = mode === "signup";
  const shownError = state?.error ?? initialError;
  const switchHref = `${isSignup ? "/login" : "/signup"}?next=${encodeURIComponent(next)}`;

  if (!authEnabled) {
    return (
      <div className="rounded-2xl border border-line bg-paper p-6 text-center">
        <p className="text-3xl" aria-hidden>
          🌱
        </p>
        <h1 className="mt-2 font-display text-xl font-extrabold text-ink">
          Accounts are coming soon
        </h1>
        <p className="mt-2 text-sm leading-relaxed text-muted">
          For now your progress is saved right here on this device — no sign-in
          needed. Jump in and start learning.
        </p>
        <Link
          href={next}
          className="mt-5 inline-flex rounded-full bg-coral px-6 py-3 font-display text-sm font-bold text-white transition hover:bg-coral-deep"
        >
          Start learning →
        </Link>
      </div>
    );
  }

  return (
    <div className="rounded-2xl border border-line bg-paper p-6">
      <h1 className="font-display text-2xl font-extrabold tracking-tight text-ink">
        {isSignup ? "Create your account" : "Welcome back"}
      </h1>
      <p className="mt-1 text-sm text-muted">
        {isSignup
          ? "Save your progress and earn your diploma."
          : "Pick up right where you left off."}
      </p>

      {googleEnabled && (
        <>
          <a
            href={`/api/auth/google?next=${encodeURIComponent(next)}`}
            className="mt-5 flex w-full items-center justify-center gap-2.5 rounded-xl border border-line bg-cream/40 px-4 py-2.5 text-sm font-semibold text-ink transition hover:border-coral/40"
          >
            <GoogleMark />
            Continue with Google
          </a>
          <div className="my-5 flex items-center gap-3 text-xs text-muted">
            <span className="h-px flex-1 bg-line" />
            or
            <span className="h-px flex-1 bg-line" />
          </div>
        </>
      )}

      <form action={formAction} className={googleEnabled ? "space-y-3" : "mt-5 space-y-3"}>
        <input type="hidden" name="next" value={next} />

        {isSignup && (
          <Field
            label="Name (optional)"
            name="name"
            placeholder="What should we call you?"
            autoComplete="name"
          />
        )}

        {isSignup ? (
          <Field
            label="Username"
            name="username"
            placeholder="3+ characters"
            autoComplete="username"
            required
          />
        ) : (
          <Field
            label="Username or email"
            name="identifier"
            placeholder="you"
            autoComplete="username"
            required
          />
        )}

        {isSignup && (
          <Field
            label="Email"
            name="email"
            type="email"
            placeholder="so you can use Google later"
            autoComplete="email"
            required
          />
        )}

        <Field
          label="Password"
          name="password"
          type="password"
          placeholder={isSignup ? "at least 8 characters" : "your password"}
          autoComplete={isSignup ? "new-password" : "current-password"}
          required
        />

        {shownError && (
          <p className="rounded-xl bg-coral/10 px-3 py-2.5 text-sm font-medium text-coral-deep">
            {shownError}
          </p>
        )}

        <button
          type="submit"
          disabled={pending}
          className="w-full rounded-full bg-coral px-6 py-3 font-display text-sm font-bold text-white transition enabled:hover:bg-coral-deep disabled:cursor-not-allowed disabled:opacity-50"
        >
          {pending ? "One moment…" : isSignup ? "Create account" : "Log in"}
        </button>
      </form>

      <p className="mt-5 text-center text-sm text-muted">
        {isSignup ? "Already have an account?" : `New to ${BRAND}?`}{" "}
        <Link href={switchHref} className="font-bold text-coral hover:text-coral-deep">
          {isSignup ? "Log in" : "Create one"}
        </Link>
      </p>
    </div>
  );
}

function Field({
  label,
  name,
  type = "text",
  placeholder,
  autoComplete,
  required,
}: {
  label: string;
  name: string;
  type?: string;
  placeholder?: string;
  autoComplete?: string;
  required?: boolean;
}) {
  return (
    <label className="block">
      <span className="text-xs font-semibold text-muted">{label}</span>
      <input
        name={name}
        type={type}
        placeholder={placeholder}
        autoComplete={autoComplete}
        required={required}
        className="mt-1 w-full rounded-xl border border-line bg-cream/40 px-4 py-2.5 text-ink outline-none transition focus:border-coral"
      />
    </label>
  );
}

function GoogleMark() {
  return (
    <svg viewBox="0 0 24 24" className="size-4" aria-hidden>
      <path
        fill="#4285F4"
        d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.77h3.57c2.08-1.92 3.27-4.74 3.27-8.1z"
      />
      <path
        fill="#34A853"
        d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.99.66-2.26 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84A11 11 0 0 0 12 23z"
      />
      <path
        fill="#FBBC05"
        d="M5.84 14.1a6.6 6.6 0 0 1 0-4.2V7.06H2.18a11 11 0 0 0 0 9.88l3.66-2.84z"
      />
      <path
        fill="#EA4335"
        d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1A11 11 0 0 0 2.18 7.06l3.66 2.84C6.71 7.3 9.14 5.38 12 5.38z"
      />
    </svg>
  );
}
